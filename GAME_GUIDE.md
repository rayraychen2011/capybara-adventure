...
2. **🏗️ 完整碰撞系統**：玩家與建築物、NPC 與建築物均有完善的碰撞檢測
3. **🌊 水域限制**：NPC 無法進入水體區域，確保合理的移動路徑
...
1. **裝備槍械**：按 `中鍵` 開啟武器圓盤，選擇槍械（按 `1` 鍵）
2. **進入狩獵模式**：按 `G` 鍵啟動狩獵模式
3. **尋找目標**：在森林或湖泊區域尋找野生動物

### 狩獵流程

1. **瞄準動物**：將滑鼠移到動物附近進行瞄準
2. **等待鎖定**：保持瞄準 1 秒以上讓準星變綠
3. **射擊**：按 `左鍵` 進行射擊
4. **獲得獎勵**：成功獵殺後獲得金錢和經驗
...
- **普通動物**：基礎獎勵 $50-100
- **稀有動物**：獎勵 $150-200，看到玩家會逃跑
- **超稀有動物**：獎勵 $300-400，較少見且警覺
- **傳奇動物**：獎勵 $1000+，有領地行為，會反擊
...
1. **裝備斧頭**：按 `中鍵` 開啟武器圓盤，選擇斧頭（按 `2` 鍵）
2. **靠近樹木**：走到森林區域的樹木旁邊
3. **砍伐**：按 `Q` 鍵砍伐樹木
4. **獲得木材**：每棵樹木提供 $20 的木材獎勵
